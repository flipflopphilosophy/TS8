...
def save_job_posts_to_db(job_posts):
    ...
    for job in job_posts:
        ...
        """, (
                    json.dumps(job.get('id', 'NA')), 
                    json.dumps(job.get('company_name', 'NA')), 
                    json.dumps(job.get('role', 'NA')), 
                    json.dumps(job.get('level', 'NA')), 
                    json.dumps(job.get('salary', 'NA')), 
                    json.dumps(job.get('salary_median', 'NA')), 
                    json.dumps(job.get('location', 'NA')), 
                    json.dumps(job.get('state', 'NA')), 
                    json.dumps(job.get('setting', 'NA')), 
                    json.dumps(job.get('in_state', 'NA')), 
                    json.dumps(job.get('industry', 'NA')), 
                    json.dumps(job.get('public', 'NA')), 
                    json.dumps(job.get('employee_count', 'NA')), 
                    json.dumps(job.get('revenue', 'NA')), 
                    json.dumps(job.get('benefits', 'NA')), 
                    json.dumps(job.get('relocation', 'NA')), 
                    json.dumps(job.get('undergraduate', 'NA')), 
                    json.dumps(job.get('postgraduate', 'NA')), 
                    json.dumps(job.get('experience', 'NA')), 
                    json.dumps(job.get('fields', 'NA')), 
                    json.dumps(job.get('cpa', 'NA')), 
                    json.dumps(job.get('certifications', 'NA')), 
                    json.dumps(job.get('strategic_responsibilities', 'NA')), 
                    json.dumps(job.get('operational_responsibilities', 'NA')), 
                    json.dumps(job.get('team_management', 'NA')), 
                    json.dumps(job.get('budgetary_control', 'NA')), 
                    json.dumps(job.get('decision_making_authority', 'NA')), 
                    json.dumps(job.get('reporting_structure', 'NA')), 
                    json.dumps(job.get('cross_functional_interaction', 'NA')), 
                    json.dumps(job.get('company_culture', 'NA')), 
                    json.dumps(job.get('technology_proficiency', 'NA')), 
                    json.dumps(job.get('stakeholder_engagement', 'NA')), 
                    json.dumps(job.get('travel_requirements', 'NA')), 
                    json.dumps(job.get('succession_planning', 'NA')), 
                    json.dumps(job.get('crisis_management', 'NA')), 
                    json.dumps(job.get('leadership_style', 'NA')), 
                    json.dumps(job.get('change_management', 'NA')), 
                    json.dumps(job.get('risk_management', 'NA')), 
                    json.dumps(job.get('job_posting_source', 'NA')), 
                    json.dumps(job.get('date_job_posted', 'NA')), 
                    json.dumps(job.get('date_job_tracked', 'NA')), 
                    json.dumps(job.get('works_with_recruiters', 'NA')), 
                    json.dumps(job.get('company_profile', 'NA')),
                    json.dumps(job.get('soft_skills', 'NA')), 
                    json.dumps(job.get('hard_skills', 'NA')), 
                    json.dumps(job.get('job_description_summary', 'NA')), 
                    json.dumps(job.get('other_requirements', 'NA'))
        ))
    ...
    try:
        ...
        for candidate_dict in candidate_profiles:
            ...
            if isinstance(candidate_dict, dict):
                ...
                """, (
                    json.dumps(candidate_dict.get('id', 'NA')),
